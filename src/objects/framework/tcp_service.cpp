...
void tcp_service::set_running(bool running) { running_.store(running, std::memory_order_release); }

void tcp_service::add(shared_tcp_connection writer) {
...
vector_of<shared_tcp_connection> tcp_service::snapshot() {
  std::lock_guard lock(mutex_);
  return writers_;
}
}  // namespace framework
