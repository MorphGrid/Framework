...
namespace framework {
tcp_endpoint::tcp_endpoint(const uuid id, const unsigned short int port,
                           shared_of<tcp_handlers<tcp_endpoint, tcp_connection<tcp_endpoint>>> handlers)
    : id_(id), port_(port), callback_(std::move(handlers)) {}

shared_of<tcp_handlers<tcp_endpoint, tcp_connection<tcp_endpoint>>> tcp_endpoint::handlers() const { return callback_; }
...
void tcp_endpoint::add(shared_of<tcp_connection<tcp_endpoint>> writer) {
...
void tcp_endpoint::remove(uuid session_id) {
  std::lock_guard lock(mutex_);
  std::erase_if(writers_, [&session_id](const shared_of<tcp_connection<tcp_endpoint>>& w) { return w->get_id() == session_id; });
}

vector_of<shared_of<tcp_connection<tcp_endpoint>>> tcp_endpoint::snapshot() {
  std::lock_guard lock(mutex_);
  return writers_;
}
}  // namespace framework
