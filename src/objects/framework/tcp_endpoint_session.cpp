...
async_of<void> notify_error_and_close(const shared_tcp_endpoint service, const shared_tcp_endpoint_connection connection,
...
async_of<void> notify_disconnected_if_present(const shared_tcp_endpoint service, const shared_tcp_endpoint_connection connection) {
  if (service->handlers()->on_disconnected()) co_await service->handlers()->on_disconnected()(service, connection);
  service->remove(connection->get_id());
  co_return;
}
...
async_of<void> tcp_endpoint_session(const shared_state state, const shared_tcp_endpoint service,
                                    const shared_tcp_endpoint_connection connection) {
