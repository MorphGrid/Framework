#ifndef FRAMEWORK_TASK_GROUP_HPP
#define FRAMEWORK_TASK_GROUP_HPP
...
class task_group {
 ...
 public:
  explicit task_group(boost::asio::any_io_executor exec)
  ...
  auto adapt(CompletionToken&& completion_token) {
    ...
    class remover {
     ...
     public:
      remover(task_group* tg,
              std::list<boost::asio::cancellation_signal>::iterator cs)
          : task_group_{tg}, signal_{cs} {}

      remover(remover&& other) noexcept
          : task_group_{std::exchange(other.task_group_, nullptr)},
            signal_{other.signal_} {}

      ~remover() {
        if (task_group_) {
          auto _lock = std::lock_guard{task_group_->mutex_};
          if (task_group_->signals_.erase(signal_) ==
              task_group_->signals_.end()) {
            try {
  ...
  auto async_wait(CompletionToken&& completion_token =
    ...
    return boost::asio::async_compose<CompletionToken,
        ...
        [this, _scheduled = false](auto&& self,
          ...
          if (!std::exchange(_scheduled, true))
...
#endif  // FRAMEWORK_TASK_GROUP_HPP
