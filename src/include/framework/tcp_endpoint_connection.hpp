...
#ifndef FRAMEWORK_TCP_ENDPOINT_CONNECTION_HPP
#define FRAMEWORK_TCP_ENDPOINT_CONNECTION_HPP

#include <framework/auth.hpp>
#include <framework/support.hpp>

namespace framework {
class tcp_endpoint_connection : public std::enable_shared_from_this<tcp_endpoint_connection> {
  boost::asio::streambuf buffer_;
  shared_tcp_endpoint service_;
  shared_auth auth_ = std::make_shared<auth>();

 public:
  tcp_endpoint_connection(uuid id, shared_of<tcp_executor> strand, shared_of<tcp_stream> stream, shared_tcp_endpoint service);
...
#endif  // FRAMEWORK_TCP_ENDPOINT_CONNECTION_HPP
